      ...
      if (step > 0) {
        for (i = 0; i < n; ++i) {
          if ((x4 = values[i]) != null && x06 <= x4 && x4 <= x12) {
            bins[Math.min(m2, Math.floor((x4 - x06) / step))].push(data[i]);
          }
        }
      } else if (step < 0) {
        for (i = 0; i < n; ++i) {
          if ((x4 = values[i]) != null && x06 <= x4 && x4 <= x12) {
            ...
            bins[Math.min(m2, j + (tz[j] <= x4))].push(data[i]);
          }
        }
      }
    } else {
...
function thresholdFreedmanDiaconis(values, min4, max5) {
  ...
  return c5 && d ? Math.ceil((max5 - min4) / (2 * d * Math.pow(c5, -1 / 3))) : 1;
}

// node_modules/d3-array/src/threshold/scott.js
function thresholdScott(values, min4, max5) {
  const c5 = count(values), d = deviation(values);
  return c5 && d ? Math.ceil((max5 - min4) * Math.cbrt(c5) / (3.49 * d)) : 1;
}
...
function transpose(matrix) {
  ...
  for (var i = -1, m2 = min(matrix, length2), transpose2 = new Array(m2); ++i < m2; ) {
...
function genericArray(a3, b) {
  ...
  for (i = 0; i < na; ++i) x4[i] = value_default(a3[i], b[i]);
  for (; i < nb; ++i) c5[i] = b[i];
  return function(t) {
    for (i = 0; i < na; ++i) c5[i] = x4[i](t);
    return c5;
  };
}
...
function density_default() {
  var x4 = defaultX, y4 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m2 = dy + o * 2 >> k2, threshold2 = constant_default6(20);
  function grid(data) {
  ...
  function density(data) {
    ...
    return contours_default().size([n, m2]).thresholds(tz.map((d) => d * pow4k))(values).map((c5, i) => (c5.value = +tz[i], transform2(c5)));
  }
  ...
  *neighbors(i) {
  ...
  _clipSegment(x06, y06, x12, y12, c0, c1) {
    ...
    while (true) {
      ...
      let x4, y4, c5 = c0 || c1;
      if (c5 & 8) x4 = x06 + (x12 - x06) * (this.ymax - y06) / (y12 - y06), y4 = this.ymax;
      else if (c5 & 4) x4 = x06 + (x12 - x06) * (this.ymin - y06) / (y12 - y06), y4 = this.ymin;
      else if (c5 & 2) y4 = y06 + (y12 - y06) * (this.xmax - x06) / (x12 - x06), x4 = this.xmax;
  ...
  _project(x06, y06, vx, vy) {
    ...
    if (vy < 0) {
      ...
      if ((c5 = (this.ymin - y06) / vy) < t) y4 = this.ymin, x4 = x06 + (t = c5) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax) return null;
      if ((c5 = (this.ymax - y06) / vy) < t) y4 = this.ymax, x4 = x06 + (t = c5) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax) return null;
      if ((c5 = (this.xmax - x06) / vx) < t) x4 = this.xmax, y4 = y06 + (t = c5) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin) return null;
      if ((c5 = (this.xmin - x06) / vx) < t) x4 = this.xmin, y4 = y06 + (t = c5) * vy;
    }
    return [x4, y4];
  }
  _edgecode(x4, y4) {
...
var Delaunay = class _Delaunay {
  ...
  *neighbors(i) {
  ...
  find(x4, y4, i = 0) {
    ...
    const i0 = i;
    let c5;
    while ((c5 = this._step(i, x4, y4)) >= 0 && c5 !== i && c5 !== i0) i = c5;
    return c5;
  }
  _step(i, x4, y4) {
...
function autoType(object2) {
  for (var key in object2) {
    ...
    if (!value) value = null;
      ...
      if (fixtz && !!m2[4] && !m2[7]) value = value.replace(/-/g, "/").replace(/T/, " ");
...
function link_default(links) {
  var id2 = index2, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count2, bias, random, iterations2 = 1;
  if (links == null) links = [];
...
function areaPointFirst(lambda, phi) {
  ...
  lambda0 = lambda, cosPhi0 = cos2(phi = phi / 2 + quarterPi), sinPhi0 = sin2(phi);
}
function areaPoint(lambda, phi) {
  lambda *= radians2, phi *= radians2;
  phi = phi / 2 + quarterPi;
  var dLambda = lambda - lambda0, sdLambda = dLambda >= 0 ? 1 : -1, adLambda = sdLambda * dLambda, cosPhi = cos2(phi), sinPhi = sin2(phi), k2 = sinPhi0 * sinPhi, u4 = cosPhi0 * cosPhi + k2 * cos2(adLambda), v2 = k2 * sdLambda * sin2(adLambda);
...
function centroidLinePoint(lambda, phi) {
  lambda *= radians2, phi *= radians2;
  var cosPhi = cos2(phi), x4 = cosPhi * cos2(lambda), y4 = cosPhi * sin2(lambda), z = sin2(phi), w = atan2(sqrt((w = y0 * z - z0 * y4) * w + (w = z0 * x4 - x0 * z) * w + (w = x0 * y4 - y0 * x4) * w), x0 * x4 + y0 * y4 + z0 * z);
  W1 += w;
  X1 += w * (x0 + (x0 = x4));
  Y1 += w * (y0 + (y0 = y4));
  Z1 += w * (z0 + (z0 = z));
...
function centroidRingPoint(lambda, phi) {
  lambda *= radians2, phi *= radians2;
  var cosPhi = cos2(phi), x4 = cosPhi * cos2(lambda), y4 = cosPhi * sin2(lambda), z = sin2(phi), cx = y0 * z - z0 * y4, cy = z0 * x4 - x0 * z, cz = x0 * y4 - y0 * x4, m2 = hypot(cx, cy, cz), w = asin(m2), v2 = m2 && -w / m2;
  X2.add(v2 * cx);
  Y2.add(v2 * cy);
  Z2.add(v2 * cz);
  W1 += w;
...
function compose_default(a3, b) {
  ...
  if (a3.invert && b.invert) compose.invert = function(x4, y4) {
...
function rotationPhiGamma(deltaPhi, deltaGamma) {
  ...
  function rotation(lambda, phi) {
    var cosPhi = cos2(phi), x4 = cos2(lambda) * cosPhi, y4 = sin2(lambda) * cosPhi, z = sin2(phi), k2 = z * cosDeltaPhi + x4 * sinDeltaPhi;
    return [
  ...
  rotation.invert = function(lambda, phi) {
    var cosPhi = cos2(phi), x4 = cos2(lambda) * cosPhi, y4 = sin2(lambda) * cosPhi, z = sin2(phi), k2 = z * cosDeltaGamma - y4 * sinDeltaGamma;
    return [
...
function polygonContains_default(polygon, point6) {
  ...
  var sum4 = new Adder();
  if (sinPhi === 1) phi = halfPi3 + epsilon7;
  else if (sinPhi === -1) phi = -halfPi3 - epsilon7;
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi) {
        var point1 = [lambda, phi], point22, v2 = visible(lambda, phi), c5 = smallRadius ? v2 ? 0 : code(lambda, phi) : v2 ? code(lambda + (lambda < 0 ? pi4 : -pi4), phi) : 0;
...
function lengthPointFirst(lambda, phi) {
  lambda *= radians2, phi *= radians2;
  lambda03 = lambda, sinPhi02 = sin2(phi), cosPhi02 = cos2(phi);
  lengthStream.point = lengthPoint;
}
function lengthPoint(lambda, phi) {
  lambda *= radians2, phi *= radians2;
  var sinPhi = sin2(phi), cosPhi = cos2(phi), delta = abs3(lambda - lambda03), cosDelta = cos2(delta), sinDelta = sin2(delta), x4 = cosPhi * sinDelta, y4 = cosPhi02 * sinPhi - sinPhi02 * cosPhi * cosDelta, z = sinPhi02 * sinPhi + cosPhi02 * cosPhi * cosDelta;
...
function resample(project, delta2) {
  ...
  return function(stream) {
    ...
    function linePoint2(lambda, phi) {
      var c5 = cartesian([lambda, phi]), p = project(lambda, phi);
...
function projectionMutator(projectAt) {
  var project, k2 = 150, x4 = 480, y4 = 250, lambda = 0, phi = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x06 = null, y06, x12, y12, postclip = identity_default3, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
  ...
  function recenter() {
    var center2 = scaleTranslateRotate(k2, 0, 0, sx, sy, alpha).apply(null, project(lambda, phi)), transform2 = scaleTranslateRotate(k2, x4 - center2[0], y4 - center2[1], sx, sy, alpha);
...
function mercatorRaw(lambda, phi) {
  return [lambda, log(tan((halfPi3 + phi) / 2))];
}
...
function equalEarthRaw(lambda, phi) {
  var l = asin(M * sin2(phi)), l2 = l * l, l6 = l2 * l2 * l2;
  return [
...
naturalEarth1Raw.invert = function(x4, y4) {
  var phi = y4, i = 25, delta;
  do {
    var phi2 = phi * phi, phi4 = phi2 * phi2;
    phi -= delta = (phi * (1.007226 + phi2 * (0.015085 + phi4 * (-0.044475 + 0.028874 * phi2 - 5916e-6 * phi4))) - y4) / (1.007226 + phi2 * (0.015085 * 3 + phi4 * (-0.044475 * 7 + 0.028874 * 9 * phi2 - 5916e-6 * 11 * phi4)));
  } while (abs3(delta) > epsilon7 && --i > 0);
  return [
    x4 / (0.8707 + (phi2 = phi * phi) * (-0.131979 + phi2 * (-0.013791 + phi2 * phi2 * phi2 * (3971e-6 - 1529e-6 * phi2)))),
    phi
  ];
};
...
function transverseMercatorRaw(lambda, phi) {
  return [log(tan((halfPi3 + phi) / 2)), -lambda];
}
...
function pie_default() {
  ...
  function pie(data) {
    var i, n = (data = array_default2(data)).length, j, k2, sum4 = 0, index3 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau6, Math.max(-tau6, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
...
function zoom_default2() {
  ...
  function schedule(transition2, transform2, point6, event) {
    transition2.on("start.zoom", function() {
      ...
      var that = this, args = arguments, g = gesture(that, args).event(event), e = extent2.apply(that, args), p = point6 == null ? centroid(e) : typeof point6 === "function" ? point6.apply(that, args) : point6, w = Math.max(e[1][0] - e[0][0], e[1][1] - e[0][1]), a3 = that.__zoom, b = typeof transform2 === "function" ? transform2.apply(that, args) : transform2, i = interpolate(a3.invert(p).concat(w / a3.k), b.invert(p).concat(w / b.k));
      return function(t) {
        if (t === 1) t = b;
        else {
          var l = i(t), k2 = w / l[2];
